...
def load_review(review, product_id, page, order):
    ...
    #loader.add_value('plaintext_review', plaintext)
    ...
    # Review data.
    loader.add_css('recommended', '.ratingSummary::text')
    loader.add_css('date', '.recommendation_date::text', re='Posted: (.+)')
    #loader.add_css('text', '.apphub_CardTextContent::text')
    loader.add_xpath('text', '//*[@id="ReviewEditTextArea"]/text()')
    #loader.add_css('hours', '.hours::text', re='(.+) hrs')
    #loader.add_css('compensation', '.received_compensation::text')

    # User/reviewer data.
    loader.add_css('user_id', '.profile_small_header_name a::attr(href)', re='.*/profiles/(.+)/')
    loader.add_css('username', '.profile_small_header_name a::text')
...
class ReviewSpider(scrapy.Spider):
    ...
    test_urls = [
        # Full Metal Furies
        'http://steamcommunity.com/app/570/reviews/?browsefilter=mostrecent&p=1&filterLanguage=english'
    ]
    ...
    def parse_review(self, response):
        ...
        #plaintext = response.xpath('//*[@id="ReviewEditTextArea"]/text()').extract_first().strip()
        yield load_review(response, response.meta['product_id'], response.meta['page'], response.meta['i'])
    ...
    def parse(self, response):
        ...
        for i, review in enumerate(reviews):
            plaintext_review_lnk = review.css('div::attr(data-modal-content-url)').extract_first()
            #print('PLAINTEXT LINK: {}'.format(plaintext_review_lnk))
            request = Request(plaintext_review_lnk, callback=self.parse_review)
